...
In this representation we can see that the spatial infinity, $i^0$, is mapped to the origin ($T = 0$ and $\psi = 0$) and $\mathscr{I}^{\pm}$ and to the future/past null cone through the origin.\\
\begin{figure}[h!]
...
Let $S$ be a sphere of radius r with the following line element,
...
\begin{align}\label{NP1_precomputation}
  ...
  & = (1+\tau)^{-4}\left(\sum_{p=0}^{1} \frac{1}{p !} \rho^{p-2} Q_{p; 1, m}^{1}(\tau)+\sum_{p=2}^{2} \frac{1}{p !} \rho^{0} Q_{p, 1, m}^{1}(\tau)+\sum_{p=3}^{\infty} \frac{1}{p !} \rho^{p-2} A_{p, 1, m}^{1}\cdot{(\tau)}\right) = \nonumber \\
  & = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow 1}} 2^{-4}\big\{\rho^{-2} Q_{0,1, m}^{1}(\tau)+\rho^{-1} Q_{1,1, m}^{1}(\tau)\big\}+\lim_{\substack{\rho \rightarrow 0 \\ \tau \rightarrow 1}} 2^{-4} \frac{1}{2 !} Q^{1}_{2, 1, m}(\tau). 
\end{align}
...
\begin{align}
  \mathcal{N}^{-}_{0,0}= \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}}  \underline{\boldsymbol{e}^{-}}(\phi_{00}) = \sum_{p=0}^{\infty} \frac{1}{p!}\rho^{p-1}_{\star}\underline{Q}^{0}_{p,0,0}(\tau)|_{\mathscr{I}^{-}}.
\end{align}
...
\begin{align}\label{NP1-_precomputation}
  & \mathcal{N}^{-}_{1,m}= \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}} (\underline{\boldsymbol{e}^{-}})^2(\phi_{1m}) = \sum_{p=1}^{\infty}\frac{1}{p!}\rho_{\star}^{p-2}\underline{Q}^{1}_{p,1,m}(\tau)|_{\mathscr{I}^{-}} =  \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}}(1-\tau)^{-4} \sum_{p=0}^{\infty} \frac{1}{p !} \underline{Q}_{p \ell, m}^{1}(\tau) \rho^{p} = \nonumber \\
  & = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}}(1-\tau)^{-4}\left(\sum_{p=0}^{1} \frac{1}{p !} \rho^{p-2} \underline{Q}_{p; 1, m}^{1}(\tau)+\sum_{p=2}^{2} \frac{1}{p !} \rho^{0} \underline{Q}_{p, 1, m}^{1}(\tau)+\sum_{p=3}^{\infty} \frac{1}{p !} \rho^{p-2} \underline{Q}_{p, 1, m}^{1}{(\tau)}\right) = \nonumber \\
  & = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}} 2^{-4}\big\{\rho^{-2} \underline{Q}_{0,1, m}^{1}(\tau)+\rho^{-1} \underline{Q}_{1,1, m}^{1}(\tau)\big\}+\lim_{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}} 2^{-4} \frac{1}{2 !} \underline{Q}^{1}_{2, 1, m}(\tau). 
\end{align}
Similar to the previous case, we have the following:
