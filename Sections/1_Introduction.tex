...
\end{equation}
which is the metric of the Einstein static universe. The conformal boundary is given by $\Xi = \cos T + \cos \psi$, where the cylinder $\mathbb{R} \times \mathbb{S}^2$ is the Einstein Cylinder. The purpose of this thesis is  to study what happens at infinity, and in order to do that we will focus on the region where $\Xi = 0$. This condition gives us the following regions, which are presented in the following table
\begin{center}
    \begin{tabular}{ |c|c|c| }
      \hline
      Region & Name & Symbol \\
     ...
     \hline 
     $\tilde{r} \rightarrow $<$ \infty$  \;\; with \;\; $\tilde{t} \rightarrow \pm \infty$
     \enspace & Future/Past Timelike Infinity & $i^{\pm}$ \\
     \hline
     $\tilde{r} \rightarrow \infty, \enspace \tilde{t} \rightarrow \infty \enspace
     \text{with} \;\; |\tilde{u}|$<$\infty$ &  Future Null-infinity & $\mathscr{I}^+$ \\ 
     \hline
     $\tilde{r} \rightarrow\infty, \enspace \tilde{t} \rightarrow-\infty \;\;
     \text{with} \enspace |\tilde{v}|$<$\infty$ & Past Null-infinity & $\mathscr{I}^-$ \\
     \hline
    \end{tabular}
    \end{center}
...
In this representation we can see that the spatial infinity, $i^0$, is mapped to the origin ($T = 0$ and $\psi = \pi$) and $\mathscr{I}^{\pm}$ and to the future/past null cone through the origin.\\
\begin{figure}[h!]
...
\begin{align}\label{eq:tetrad}
  ...
  & m^{\mu} = \displaystyle\frac{1}{r}\left(\delta_{2}^{\mu} + \displaystyle\frac{i}{\sin\theta}\ \delta_{3}^{\mu}\right), \qquad \overline{m}^{\mu} = \displaystyle\frac{1}{r}\left(\delta_{2}^{\mu} - \displaystyle\frac{i}{\sin\theta}\ \delta_{3}^{\mu}\right).
\end{align}
...
\end{align}
From \eqref{eq:Maxtensor} we can see that $\Phi_{0}$, $\Phi_{1}$ and $\Phi_{2}$ have spin weights of 1, 0 and -1, respectively.\\
Let $S$ be a $\mathbb{S}^2$ sphere of radius r with the following line element,
\begin{equation}\label{eq:lineelement}
  ds^{2} = -r^{2}(d\theta^{2} + \sin^{2}\theta d\phi^{2}) = -2m _{(a}\overline{m}_{b)}dx^{a}dx^{b}.
\end{equation}
Note that can be the following gauge freedom,
\begin{equation}\label{eq:gaugefreedom}
  m \rightarrow \hat{m} = e^{i\psi}m, \quad \overline{m} \rightarrow \hat{\overline{m}} = e^{-i\psi}\overline{m},
\end{equation}
where $\psi$ is a real function. This represents a spin through $\psi$. Let $\eta_{a...by...z}$ be a tensor in $S$, where there are $p$ indices in the first index set and $q$ indices in the second. Let
\begin{equation}
  \eta = m^{a}...m^{b}\overline{m}^{y}...\overline{m}^{z}\eta_{a...by...z}.
\end{equation}
Under spins \eqref{eq:gaugefreedom} $\eta$ transforms as follows,
\begin{equation}
  \eta \rightarrow \hat{\eta} = e^{is\psi}\eta,
\end{equation}
where $s = p - q$ is the spin weight of $\eta$.\\
Thus, we can write the Maxwell equations in terms of $\Phi$, the null polar coordinates and the operator $\dh$ in the following way,
...
\begin{equation}\label{eq:spinweightSH}
  { }_s Y_{l m}(\theta, \phi)= \begin{cases}(-1)^s \left[2^s \frac{(l-s) !}{(l+s) !}\right]^{1 / 2} \dh^s Y_{l m}(\theta, \phi) & 0 \leq s \leq l \\ \left[2^{-s} \frac{(l+s) !}{(l-s) !}\right]^{1 / 2} \overline{\dh}^{-s} Y_{l m}(\theta, \phi) & 0 \geq s \geq-l \\ 0 & \text { otherwise. }\end{cases}
\end{equation}
\\
...
\end{equation}
where the over dot represents the derivative with respect to $u$.\\
...
\end{equation}
with $\int{_{s}Y_{l,m}\dh^{l-s+1}\xi d\omega} = 0$ \cite{NewPen68}. Therefore, $\int {_{0}Y_{0,0}\Phi_{1}^{0} d\omega}$ is constant and proportional to the total electric charge.\\
...
\begin{equation}\label{eq:Psi0ansatz}
  \Psi_{0}=\sum_{n=0}^{N}\frac{\Psi_{0}^{n}}{r^{5+n}}+o(r^{-5-N}),
\end{equation}
...
quantities.
Therefore, the study of the NP constants and their conservation offers valuable insights into the
...
Hence, the peeling theorem and the NP constants are interconnected concepts that deepen our
...
The Peeling theorem underscores the advantages of adopting a gauge that is well-suited to the structure of null infinity. By utilizing the NP gauge, we gain even deeper insights into the properties of asymptotically simple spacetimes. This gauge choice allows for a more comprehensive understanding of the behavior and characteristics of these spacetimes at infinity \cite{Val16}. 
...
\end{equation}
The unphysical metric $\boldsymbol{g}$, also known as the $i^0$ - cylinder metric, is associated with the coordinates $(\tau, \rho, \vartheta^A)$, which are commonly referred to as the $F$ - coordinates system, where $F$ stands for Friedrich.
...
\begin{figure}[h!]
	\centering \includegraphics[width =0.28\textwidth]{friedrich cylinder.pdf}
...
In \cite{NewPen68}, the NP constants were originally defined within the linear framework for spin-1 and spin-2 fields, as well as within the nonlinear setting for the Einstein field equations formulated in the NP formalism.
...
\end{align}
\begin{proposition}\label{prop:operator_phi} The following expressions will be inductively proved by using \eqref{Lemma:JacobiPoly}.
  \begin{align}
...
\end{align}
\begin{proof}
  To prove \eqref{prop:operator_phi}, we proceed inductively as follows. We first verify that equation (2.36) when substituting $n = 1$ and $n = 2$, taking $A = \tau\partial_{\tau}\partial_{\rho}\phi$, gives the following:
  ...
  \begin{lemma}\label{Lemma:JacobiPoly}
  Assuming equation (2.36) is valid for $n = N$ (induction hypothesis), 
  \begin{equation}\label{eq:induction_hypothesis}
    \partial_{\rho}^{N}(\rho A) = \rho\partial^{N}_{\rho}A + N\partial^{N-1}_{\rho}A.
  \end{equation}
  we will show that it is also valid for $n = N + 1$ (induction step). We have:
  ...
  \end{align}
\end{lemma}
  For equation (2.37), we proceed in a similar manner. Taking $B = \rho C$, gives the following:
  \begin{align}
...
\begin{align}\label{eq:phi_expansion_2}
  ...
  & + \frac{\rho}{16}\left(1-\tau^{2}\right)\left(D_{11-1} \log \left(\frac{1+\tau}{1-\tau}\right)Y_{1-1}+D_{110} \log \left(\frac{1+\tau}{1-\tau}\right) Y_{10}+D_{111}\log \left(\frac{1+\tau}{1-\tau}\right) Y_{11}\right).
\end{align}
Using the transformation \eqref{Ftophys} one gets,
\begin{align}\label{eq:phi_expansion_3}
  ...
  & \Leftrightarrow \phi=\left[C_{000}+\frac{1}{2} D_{000} \log \left(\frac{\tilde{\rho}+\tilde{t}}{\tilde{\rho}-\tilde{t}}\right)+\frac{1}{2\tilde{\rho}}\left(A_{100}+B_{100}\right)\right]Y_{00} + \nonumber \\
  & + \frac{1}{4\tilde{\rho}}\left(C_{11-1} Y_{1-1}+C_{110}Y_{10}+C_{111} Y_{11}\right)+\frac{1}{16\tilde{\rho}} \nonumber \\
...
\begin{align}\label{eq:phi_tilde}
  & \tilde{\phi}=\Theta \phi \Leftrightarrow \tilde{\phi}=\frac{1}{\tilde{\rho}} \phi \Leftrightarrow \tilde{\phi}=\frac{C_{000}}{\tilde{\rho}}+\frac{1}{2 \tilde{\rho}} D_{000} \log \left(\frac{\tilde{\rho}+\tilde{t}}{\tilde{\rho}-\tilde{t}}\right) Y_{00} + \frac{1}{16{\tilde{\rho}^2}} \nonumber \\
  & \left[D_{11-1}\log \left(\frac{\tilde{\rho}+\tilde{t}}{\tilde{\rho}-\tilde{t}}\right) Y_{1-1}+D_{110} \log\left(\frac{\tilde{\rho}+\tilde{t}}{\tilde{\rho}-\tilde{t}}\right) Y_{10}+D_{111} \log\left(\frac{\tilde{\rho}+\tilde{t}}{\tilde{\rho}-\tilde{t}}\right)Y_{11}\right] + \nonumber \\
...
\begin{align}\label{eq:inductionstep_commutation}
  & \underline{L} L\left(e^{+}\right)^{N+1} \phi=\underline{L} L\left\{\left(e^{+}\right)\left[\left(e^{+}\right)^{N} \phi\right]\right\}=L\left[\underline{L}\left[\left(\rho^{2} L\right)\left(e^{+}\right)^{N} \phi\right]\right]= \nonumber\\ 
  ...
  & =L\left[-2 N \tilde{\rho} L\left(e^{+}\right)^{N} \phi+(N(N+1)+\Delta)\left(e^{+}\right)^{N} \phi-2 \tilde{\rho} L\left(e^{+}\right)^{N} \phi\right]= \nonumber \\
  & =-2 N \tilde{\rho} L L\left(e^{+}\right)^{N} \phi-2 N L\left(e^{+}\right)^{N} \phi+L\left[\left(N(N+1)\right)\left(e^{+}\right)^{N} \phi\right]+L\left(\Delta\left(e^{+}\right)^{N} \phi\right) -2 \tilde{\rho} L L\left(e^{+}\right)^{N} \phi-2 L\left(e^{+}\right)^{N} \phi= \nonumber \\
  ...
  & =-2 N\tilde{\rho} L L\left(e^{+}\right)^{N} \phi-2 N L\left(e^{+}\right)^{N} \phi+N^{2} L\left(e^{+}\right)^{N} \phi+N L\left(e^{+}\right)^{N} \phi+\Delta L\left(e^{+}\right)^{N} \phi -2 \tilde{\rho} L L\left(e^{+}\right)^{N} \phi-2 L\left(e^{+}\right)^{N} \phi = \nonumber \\
...
\end{align}
here, $\rho_{\star}$ represents a constant that parametrizes the cut ${C}^{+}$, and $Q^{0}_{p,\ell,m}|_{\mathscr{I}^{+}}$ denotes the value of $Q^{0}_{p,\ell,m}$ at $\mathscr{I}^{+}$, specifically at $\tau=1$. Notably, when $\rho_{\star}=0$, it corresponds to selecting ${C}^{+}$ as ${I}^{+}$.
...
\begin{align}
  ...
  & = \lim_{\substack{\rho \to 0 \\ \tau \to 1}}\left[\frac{2^{-2}}{0 !} \rho^{-1} D_{0;0,0} +\frac{2^{-2}}{\rho !} \rho^{0} A_{1;0,0} +\sum_{p=2}^{\infty} \frac{2^{-2}}{p !} \rho^{p-1} A_{p;0,0}(1-\tau)^{p-1}\right] = -A_{100}.
\end{align}
...
\begin{align}\label{NP1_precomputation}
  & \mathcal{N}^{+}_{1,m}= \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to 1}} (\boldsymbol{e}^{+})^2(\phi_{1m}) = \sum_{p=1}^{\infty}\frac{1}{p!}\rho_{\star}^{p-2}Q^{1}_{p,1,m}|_{\mathscr{I}^{+}} =  (1+\tau)^{-4} \sum_{p=1}^{\infty} \frac{1}{p !} Q_{p \ell, m}^{1}(\tau) \rho^{p} = \nonumber \\
  & = (1+\tau)^{-4}\left(\sum_{p=1}^{1} \frac{1}{p !} \rho^{p-2} Q_{p; 1, m}^{1}(\tau)+\sum_{p=2}^{2} \frac{1}{p !} \rho^{0} Q_{p, 1, m}^{1}(\tau)+\sum_{p=3}^{\infty} \frac{1}{p !} \rho^{p-2} A_{p, 1, m}^{1}\cdot{(\tau)}\right) = \nonumber \\
  & = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow 1}} 2^{-4}\big\{0+\rho^{-1} Q_{1,1, m}^{1}(\tau)\big\}+\lim_{\substack{\rho \rightarrow 0 \\ \tau \rightarrow 1}} 2^{-4} \frac{1}{2 !} Q^{1}_{2, 1, m}(\tau). 
\end{align}
...
\begin{align}
  \mathcal{N}^{+}_{1,m} = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow 1}}2^{-4} \frac{1}{2 !} Q^{1}_{2, 1, m}(\tau) = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow 1}}2^{-4}\left(2^{2-2}(2-1)2(2+1)A_{2,1,m}(1-\tau)^{2-2}\right) = 3A_{21m}.
\end{align}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{The classical NP constants at $\mathscr{I}^{-}$}
\label{sec:classicalNPminus}

In this section, we focus on computing the classical NP constants using a similar approach to the one used in \eqref{sec:classicalNP}. Specifically, we delve into computing the classical NP constants at $\mathscr{I}^{-}$ for $\ell=0$ and $\ell=1$. This analysis is facilitated by expression \eqref{eq:ansatz}, which yields the following insights:
\begin{align}\label{eq:exp_phi_lm}
  \phi_{\ell m}= \sum_{p=\ell}^{\infty}\frac{1}{p!}a_{p;\ell,m}(\tau)\rho^{p}.
\end{align}
...
\begin{align}\label{eq:bumeplus1philm}
  \underline{\boldsymbol{e}^{-}}(\phi_{\ell m}) = 4 (\Lambda_{-})^{2}\sum_{p=\ell}^{\infty} \frac{1}{p!}\rho^{p}\underline{Q}^{0}_{p,\ell,m}(\tau).
\end{align}
...
\begin{align}
  \mathcal{N}^{-}_{0,0}= \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}}  \underline{\boldsymbol{e}^{-}}(\phi_{00}) = \sum_{p=\ell}^{\infty} \frac{1}{p!}\rho^{p-1}_{\star}\underline{Q}^{0}_{p,0,0}(\tau)|_{\mathscr{I}^{-}}.
\end{align}
here, $\rho_{\star}$ represents a constant that parametrizes the cut ${C}^{-}$, and $\underline{Q}^{0}_{p,\ell,m}|_{\mathscr{I}^{-}}$ denotes the value of $\underline{Q}^{0}_{p,\ell,m}$ at $\mathscr{I}^{-}$, specifically at $\tau=-1$. Notably, when $\rho_{\star}=0$, it corresponds to selecting ${C}^{-}$ as ${I}^{-}$.
A direct calculation using Lemma \ref{Lemma:Sol_Jacobi_and_Logs} shows the following:
...
\begin{align}
  &\mathcal{N}^{-}_{0,0}= \sum_{p=0}^{\infty} \frac{1}{p!}\rho^{p-1}_{\star}\underline{Q}^{0}_{p;0,0}(\tau)|_{\mathscr{I}^{-}} = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}} \sum_{p=0}^{\infty} \frac{1}{p !}(1-\tau)^{-2} \rho_{\star}^{p-1} \underline{Q}_{p; 0,0}^{0}(\tau) = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}}\sum_{p=0}^{\infty} \frac{1}{p !} 2^{-2} \rho_{\star}^{p-1} \underline{Q}_{p; 0,0}^{0}(\tau) = \nonumber \\
...
Utilizing the aforementioned equations yields:
\begin{align}\label{eq:defQu1}
  & (\underline{\boldsymbol{e}^{-}})^2(\phi_{\ell m})= 4^2(\Lambda_{-})^{4}\sum_{p=\ell}^{\infty} \frac{1}{p!}\big(\underline{\boldsymbol{e}^{-}}\left(\rho^{p} \underline{Q}_{p; \ell, m}^{0}(\tau)\right)+\underline{Q}_{p; \ell, m}^{0}(\tau) \rho^{p}\big) = \nonumber \\
  & = 4^2(\Lambda_{-})^{4}\sum_{p=\ell}^{\infty} \frac{1}{p!}\left[(1-\tau) \rho^{p} \underline{Q}_{p; \ell, m}^{0}(\tau)+\rho^{p} \underline{Q}_{p; \ell, m}^{0}(\tau) p \rho^{p-1}\right]+\rho^{p} \underline{Q}_{p; \ell, m}^{0}(\tau) = \nonumber \\
...
\begin{align}\label{NP1-_precomputation}
  & \mathcal{N}^{-}_{1,m}= \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}} (\underline{\boldsymbol{e}^{-}})^2(\phi_{1m}) = \sum_{p=1}^{\infty}\frac{1}{p!}\rho_{\star}^{p-2}\underline{Q}^{1}_{p,1,m}(\tau)|_{\mathscr{I}^{-}} =  \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}}(1-\tau)^{-4} \sum_{p=1}^{\infty} \frac{1}{p !} \underline{Q}_{p \ell, m}^{1}(\tau) \rho^{p} = \nonumber \\
  & = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}}(1-\tau)^{-4}\left(\sum_{p=1}^{1} \frac{1}{p !} \rho^{p-2} \underline{Q}_{p; 1, m}^{1}(\tau)+\sum_{p=2}^{2} \frac{1}{p !} \rho^{0} \underline{Q}_{p, 1, m}^{1}(\tau)+\sum_{p=3}^{\infty} \frac{1}{p !} \rho^{p-2} \underline{Q}_{p, 1, m}^{1}{(\tau)}\right) = \nonumber \\
  & = \lim _{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}} 2^{-4}\big\{0+\rho^{-1} \underline{Q}_{1,1, m}^{1}(\tau)\big\}+\lim_{\substack{\rho \rightarrow 0 \\ \tau \rightarrow -1}} 2^{-4} \frac{1}{2 !} \underline{Q}^{1}_{2, 1, m}(\tau). 
\end{align}
Similar to the previous case, we have the following:
\begin{remark}\label{rem:lu1}
  By directly applying Lemma \ref{Lemma:Sol_Jacobi_and_Logs}, we can calculate: 
 \begin{align}
   \underline{Q}^{1}_{1;1,m}(\tau) &=-2^{-1}D_{11m}(1-\tau)^{-1},\\
   \underline{Q}^{1}_{p;1,m}(\tau) &=-2^{2-p}(p-1)p(p+1)B_{2,p,m}(1-\tau)^{p-2} \qquad
   \text{for}\qquad p\neq 1.
 \end{align}
\end{remark}
...
\begin{align}
  \mathcal{}^{\tilde{\rho}}\mathcal{N}^{+}_{0,0} = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to 1}} \; \varkappa^{-1}\boldsymbol{e}(\phi_{00}) = \sum_{p=0}^{\infty}\rho_{\star}^p\biggl[\frac{(1-\tau)}{(1+\tau)}Q^{0}_{p;0,0}(\tau)\biggr]|_{\mathscr{I}^{+}}.
\end{align}
Using Remark \ref{rem:l0} and evaluating at the critical set $I^{+}$, we obtain:
\begin{align}
  & \mathcal{}^{\tilde{\rho}}\mathcal{N}^{+}_{0,0} = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to 1}} \; \varkappa^{-1}\boldsymbol{e}(\phi_{00}) = \sum_{p=0}^{\infty}\rho_{\star}^p\biggl[\frac{(1-\tau)}{(1+\tau)}Q^{0}_{p;0,0}(\tau)\biggr] = \nonumber \\
...
\begin{align}
  \tilde{\rho} L (\boldsymbol{e}^{+}) \phi_{1m}= \varkappa^{-1}\boldsymbol{e} (\boldsymbol{e}^{+})\phi_{1m} = 4^{-1}\frac{(1-\tau)}{(1+\tau)}\rho^{-1}(1+\tau)^{-2}(\boldsymbol{e}^{+})^2\phi_{1m} = 4\varkappa^{-1}(\Lambda_{+})^{2}\sum_{p=1}^{\infty}\frac{1}{p!}\rho^pQ^{1}_{p;\ell,m}(\tau),
\end{align}
in the last equality, we have used equations \eqref{eq:defQ1} and \eqref{eq:DefQ1IntermsQ0}. Therefore, for $\ell=1$, we have:
\begin{align}
  \mathcal{}^{\tilde{\rho}}\mathcal{N}^{+}_{1,m} = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to 1}} \; \varkappa^{-1}\boldsymbol{e} (\boldsymbol{e}^{+}) \phi_{1m} = \sum_{p=1}^{\infty}\frac{1}{p!}\rho_{\star}^{p-1}(\varkappa^{-1}Q^{1}_{p;1,m}(\tau))|_{\mathscr{I}^{+}}.
\end{align}
\\
...
\begin{align}
  ...
  & = \lim_{\substack{\rho \to 0 \\ \tau \to 1}} \varkappa^{-1}\biggl\{\ddot{a}_{1;1,m}(\tau)+\frac{1}{2}\rho \ddot{a}_{2;1,m}(\tau) - (1+\tau)^{-1}\dot{a}_{1;1,m}(\tau)+(1+\tau)^{-1}\rho \dot{a}_{2;1,m}(\tau)- \nonumber \\
  & - \frac{1}{2}\rho(1+\tau)^{-1}\dot{a}_{2;1,m}(\tau)-(1+\tau)^{-2}\rho a_{2;1,m}(\tau)\biggr\} + O(\rho^{3}) = \nonumber \\
  & = \lim_{\substack{\rho \to 0 \\ \tau \to 1}} \frac{(1-\tau)}{(1+\tau)} \biggl\{\frac{1}{16}(\tau-1)^{-1}(1+\tau)^{-2}\bigl[16D_{11m}+7A_{21m}\rho - B_{21m}\rho + 16D_{11m}\tau - 16A_{21m}\rho\tau - 8B_{21m}\rho\tau - \nonumber \\
...
Therefore, for $\ell=0$, we have:
\begin{align}
  \mathcal{}^{\tilde{\rho}}\mathcal{N}^{-}_{0,0} = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}} \; \varkappa(\underline{\boldsymbol{e}^{-}})(\phi_{00}) = \sum_{p=0}^{\infty}\rho_{\star}^p\biggl[\frac{(1+\tau)}{(1-\tau)}\underline{Q}^{0}_{p;0,0}(\tau)\biggr]|_{\mathscr{I}^{-}}.
\end{align}
Using Remark \ref{rem:lu0} and evaluating at the critical set $I^{-}$, we obtain:
\begin{align}
  & \mathcal{}^{\tilde{\rho}}\mathcal{N}^{-}_{0,0} = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}} \; \varkappa(\underline{\boldsymbol{e}^{-}})(\phi_{00}) = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}}\sum_{p=0}^{\infty}\rho_{\star}^p\biggl[\frac{(1+\tau)}{(1-\tau)}\underline{Q}^{0}_{p;0,0}(\tau)\biggr] = \nonumber \\
...
Similarly, for $\ell=1$, the relevant quantity to evaluate is:
\begin{align}
  \tilde{\rho} \underline{L} (\underline{\boldsymbol{e}^{-}}) \phi_{1m}= \varkappa\underline{\boldsymbol{e}} (\underline{\boldsymbol{e}^{-}})\phi_{1m} = 4^{-1}\frac{(1+\tau)}{(1-\tau)}\rho^{-1}(1-\tau)^{-2}(\underline{\boldsymbol{e}^{-}})^2\phi_{1m} = 4\varkappa(\Lambda_{-})^{2}\sum_{p=1}^{\infty}\frac{1}{p!}\rho^p\underline{Q}^{1}_{p;\ell,m}(\tau),
\end{align}
in the last equality, we have used equations \eqref{eq:defQu1} and \eqref{eq:DefQu1IntermsQ0}. Therefore, for $\ell=1$, we have:
\begin{align}
  \mathcal{}^{\tilde{\rho}}\mathcal{N}^{-}_{1,m} = \lim_{\substack{\rho \to \rho_{\star} \\ \tau \to -1}} \; \varkappa\underline{\boldsymbol{e}}(\underline{\boldsymbol{e}^{-}})\phi_{1m} = \sum_{p=1}^{\infty}\frac{1}{p!}\rho_{\star}^{p-1}(\varkappa \underline{Q}^{1}_{p;1,m}(\tau))|_{\mathscr{I}^{-}}.
\end{align}
\\
...
\begin{corollary}
  $Q^{n-1}_{p;\ell,m}(\tau)$ satisfies
  \begin{align}\label{eq:Qnrecursion_coro}
    Q^{n}_{p;\ell,m}(\tau)=(1+\tau)\dot{Q}^{n-1}_{p;\ell,m}(\tau)-(p+n)Q^{n-1}_{p;\ell,m}(\tau).
  \end{align}
\end{corollary}
\medskip
\begin{remark}
...
\begin{proof}
  ...
  \begin{align}
    & Q^{0}_{p;\ell,m}(\tau)= (1+\tau)\dot{a}_{p;\ell,m} = (\ell+1)^{[1]}\Big(\frac{1+\tau}{2}\Big)^{p+1}B_{p;\ell,m}P_{\ell-1}^{(-p+1,p+1)}(\tau)+\sum_{k=0}^{1}A_{p;\ell,m}h^{0}_{k}(\tau)P_{\ell-k}^{(p+k,-p+k)}(\tau) = \nonumber \\
...
\end{align}
Now, assuming equation \eqref{eq:ind_Qpnotl} is valid as the induction hypothesis, we proceed with the induction step to compute $Q^{n}_{p;\ell,m}(\tau)$. Using equation \eqref{eq:Qnrecursion_coro}, we have:
\begin{align}
  & Q^{n}_{p;\ell,m}(\tau)=\sum_{k=0}^{n}A_{p;\ell,m} \Big[(1+\tau)\Big(h^{n-1}_{k} \dot{P}_{\ell-k}^{(p+k,-p+k)} + P_{\ell-k}^{(p+k,-p+k)}\dot{h}^{n-1}_{k} \Big) -(p+n)h^{n-1}_{k} P_{\ell-k}^{(p+k,-p+k)}\Big] + \nonumber \\ 
...
\begin{align}
  & Q^{n}_{p;\ell,m}(\tau)=\sum_{k=0}^{n}A_{p;\ell,m} \Big[\Big(\frac{1+\tau}{2}\Big)(\ell+k+1)h^{n-1}_{k} P_{\ell-k-1}^{(p+k+1,-p+k+1)}+\Big((1+\tau) \dot{h}^{n-1}_{k} -(p+n)h^{n-1}_{k}\Big)P_{\ell-k}^{(p+k,-p+k)} \Big] + \nonumber \\ 
...
General relativity is a theory of gravitation that explains the force of gravity as a curvature of spacetime caused by mass and energy. Black holes, which are objects with such strong gravitational forces that nothing, not even light, can escape them, are an important aspect of this theory. When an object falls into a black hole, it is reduced to just three numbers, leading to the loss of a large amount of information, a problem known as the "information paradox." A recent theory called "soft hair" has been proposed to explain this paradox by positing that non-trivial distortions in clocks, sensitive to the black hole's consumption history, can provide an infinite number of properties for a black hole in certain limits. The NP constants are quantities defined on null infinity in general relativity that obey conservation laws for asymptotically flat gravitational fields. These constants can be used to study the residual radiation present in spacetime after a black hole collision, and have been shown to be zero for stationary spacetimes such as the Schwarzschild and Kerr solutions. However, it is still an open question whether the NP constants are zero for all stationary spacetimes.
In this thesis, we have computed the NP constants for a spin-0 field propagating near spatial and null infinity. The analysis was carried out using prescribed analytic initial data close to $i^0$ in Minkowski spacetime, and we discussed the infinite hierarchy of NP constants.
